  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-3 md:px-4 pb-12 md:pb-16 pt-4 md:pt-6 space-y-1">
        ...
        {insights?.ml_importance && (
          <Section title="ML Pipeline Feature Importance">
            <p className="text-[0.88rem] leading-relaxed mb-3" style={{ color: 'var(--text-2)' }}>
              From a <strong>LightGBM classifier</strong> trained on TF-IDF text tokens + metadata + category labels
              (72K posts, 5-fold CV: AUC 0.78, F1 0.55). Feature importance = how much the model relies on each
              feature group <em>after controlling for everything else</em>. This is different from the raw
              correlations above — it accounts for confounders.
            </p>

            <h3 className="text-[0.75rem] uppercase tracking-wide font-semibold mb-2 mt-4"
                style={{ color: 'var(--text-3)' }}>Feature Group Breakdown</h3>
            <div className="space-y-2 mb-5">
              {Object.entries(insights.ml_importance.group_breakdown).map(([key, val]: [string, any]) => {
                const color = key === 'tfidf' ? 'var(--orange)' : key === 'metadata' ? 'var(--blue)' : 'var(--blue-mid)';
                return (
                  <div key={key} className="flex items-center gap-2 text-[0.82rem]">
                    <span className="w-24 text-right shrink-0 font-medium capitalize" style={{ color: 'var(--text-1)' }}>
                      {key}
                    </span>
                    <div className="h-[20px] rounded-sm flex-1 min-w-0" style={{ background: 'var(--surface-mid)' }}>
                      <div className="h-full rounded-sm flex items-center px-2 text-[0.65rem] font-bold text-white"
                           style={{ width: `${val.pct}%`, background: color }}>
                        {val.pct > 10 ? `${val.pct}%` : ''}
                      </div>
                    </div>
                    <span className="text-[0.68rem] w-12 shrink-0 font-semibold" style={{ color: 'var(--text-2)' }}>
                      {val.pct}%
                    </span>
                  </div>
                );
            ...
            <h3 className="text-[0.75rem] uppercase tracking-wide font-semibold mb-2"
                style={{ color: 'var(--text-3)' }}>Metadata Importance (controlling for text)</h3>
            <div className="space-y-1.5 mb-4">
              {insights.ml_importance.metadata_importance.map((m: any) => {
                const maxImp = Math.max(...insights.ml_importance.metadata_importance.map((x: any) => x.importance));
                return (
                  <div key={m.feature} className="flex items-center gap-2 text-[0.82rem]">
                    <span className="w-20 text-right shrink-0 font-medium" style={{ color: 'var(--text-1)' }}>
                      {m.feature}
                    </span>
                    <div className="h-[16px] rounded-sm flex-1 min-w-0" style={{ background: 'var(--surface-mid)' }}>
                      <div className="h-full rounded-sm" style={{
                        width: `${(m.importance / maxImp) * 100}%`,
                        background: 'var(--blue)',
                      }} />
                    </div>
                    <span className="text-[0.68rem] w-10 shrink-0" style={{ color: 'var(--text-muted)' }}>
                      {m.importance}
                    </span>
                  </div>
                );
            ...
            <h3 className="text-[0.75rem] uppercase tracking-wide font-semibold mb-2"
                style={{ color: 'var(--text-3)' }}>Category Importance</h3>
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-[0.82rem]">
              {insights.ml_importance.category_importance
                .filter((c: any) => c.importance > 0)
                .map((c: any) => (
                <span key={c.category} style={{ color: 'var(--text-2)' }}>
                  <strong>{c.category}</strong> {c.importance}
                </span>
              ))}
            </div>

            <h3 className="text-[0.75rem] uppercase tracking-wide font-semibold mb-2 mt-4"
                style={{ color: 'var(--text-3)' }}>Top Text Signals</h3>
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[0.82rem]">
              {insights.ml_importance.top_tfidf_tokens.slice(0, 12).map((t: any) => (
                <span key={t.token} style={{ color: 'var(--text-2)' }}>
                  &ldquo;{t.token}&rdquo; <span className="text-[0.68rem]" style={{ color: 'var(--text-muted)' }}>{t.importance}</span>
                </span>
              ))}
            </div>
          </Section>
