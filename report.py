...
def _start_server(port: int, directory: str):
    ...
    thread.start()
    console.print(f"[dim]HTTP server running at http://localhost:{port}[/dim]")
    return thread
...
Examples:
  ...
  python report.py --serve                # generate and serve locally
  python report.py --open                # generate, serve, and open in browser
  python report.py --open --port 9000    # custom port
  python report.py --log-level DEBUG     # verbose output for debugging
    ...
    parser.add_argument(
        "--serve",
        action="store_true",
        help="Start local HTTP server after generating report",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=_DEFAULT_PORT,
        help=f"Port for local server (default: {_DEFAULT_PORT})",
    )
    parser.add_argument(
        "--open",
        action="store_true",
        help="Open report in default browser (implies --serve)",
    )
    ...
    with Status("[bold]Analysing sentiment...", console=console) as status:
        ...
        log.debug(
            "Sentiment: avg_compound=%.3f, pos=%.1f%%, neg=%.1f%%",
        ...
        if not args.no_charts:
            ...
            try:
                ...
                console.print(
                    f"[yellow]Warning:[/yellow] Some charts could not be generated: {e}"
                )
    ...
    top_topic = (
        max(topic_data["topic_counts"], key=topic_data["topic_counts"].get, default="—")
        .replace("_", " ")
        .title()
    )
    mood = (
        "😊 Positive"
        if sentiment_data.get("avg_compound", 0) >= 0.05
        else "😢 Negative"
        if sentiment_data.get("avg_compound", 0) <= -0.05
        else "😐 Neutral"
    )
    ...
    if args.serve or args.open:
        ...
        server = _start_server(args.port, reports_dir)
        url = f"http://localhost:{args.port}/{args.date}_digest.html"
        if args.open:
            webbrowser.open(url)
    ...
    if args.serve or args.open:
        console.print(
            f"\n[bold cyan]Serving at http://localhost:{args.port}[/bold cyan] (Ctrl+C to stop)"
        )
        try:
            while True:
                pass
